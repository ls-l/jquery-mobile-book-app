...
include('include/common_function.php');   
	...
	if(isset($_REQUEST['p_ch_list']) and $_REQUEST['p_ch_list'] == 1){
		$db = db::__d();
		...
		if($totql_res > 0){
			for($i=0;$i<$totql_res;$i++){
				...
				$data_sub.= '<li><a href="chapter.html?chid='.$result[$i]["tbl_ch_id"].$p_q_s.'" data-ajax="false"><span>Ch.'.$result[$i]['tbl_ch_no'].'-</span><label>'.$result[$i]['tbl_ch_name'].'</label></a><div class="cls"></div></li>';
			}
		} else {
		   $data_sub= '';
		}
